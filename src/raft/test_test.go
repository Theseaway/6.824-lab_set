...
func TestInitialElection2A(t *testing.T) {
	...
	fmt.Println("[TEST]: checkTerms first ok")
...
func TestRejoin2B(t *testing.T) {
	servers := 3
	cfg := make_config(t, servers, false, false)
	DPrintf("TEST: t is %v", t)
	defer cfg.cleanup()

	cfg.begin("Test (2B): rejoin of partitioned leader")
	DPrintf("TEST: command --> 101")
	cfg.one(101, servers, true)

	// leader network failure
	leader1 := cfg.checkOneLeader()
	cfg.disconnect(leader1)
	DPrintf("TEST: old leader --> %d disconnect", leader1)
	...
	DPrintf("TEST: command --> 103")
	cfg.one(103, 2, true)

	// new leader network failure
	leader2 := cfg.checkOneLeader()
	cfg.disconnect(leader2)
	DPrintf("TEST: new leader --> %d disconnect", leader2)

	// old leader connected again
	cfg.connect(leader1)
	DPrintf("TEST: old leader --> %d rejoin", leader1)

	DPrintf("TEST: command --> 104")
	cfg.one(104, 2, true)

	// all together now
	cfg.connect(leader2)
	DPrintf("TEST: new leader --> %d rejoin", leader2)
...
func TestFigure82C(t *testing.T) {
	...
	DPrintf("TEST: function one finished")
	nup := servers
...
func TestFigure8Unreliable2C(t *testing.T) {
	...
	for iters := 0; iters < 1000; iters++ {
		...
		if leader != -1 && (rand.Int()%1000) < int(RaftElectionTimeout/time.Millisecond)/2 {
			...
			DPrintf("TEST : server disconnect --> %d")
			nup -= 1
		}
		...
		if nup < 3 {
			...
			if cfg.connected[s] == false {
				cfg.connect(s)
				DPrintf("TEST: server reconnect --> %d")
				nup += 1
			}
		}
	}
